...
const TeacherQuestionsPageObjectives = () => {
    ...
    // In Next.js, schoolId usually comes from the user context or a dynamic route
    // Falling back to user.schoolId if location.state isn't available
    ...
    const resetForm = () => {
        setQuestions([{ number: 1, topic: "", question: "", options: { A: "", B: "", C: "", D: "" }, answer: "" }]);
    ...
    const handleSubmitQuestions = async () => {
        const q = questions[0];
        if (!selectedTopic || !q.question.trim() || !q.answer) return toast.error("Missing data");
        ...
        try {
            ...
            if (existingDocId) {
                const docSnap = await getDoc(doc(schoollpq, "QuestionsBank", existingDocId));
            ...
            const docRef = existingDocId ? doc(schoollpq, "QuestionsBank", existingDocId) : doc(collection(schoollpq, "QuestionsBank"));
    ...
    return (
        <div className="max-w-6xl mx-auto p-6 bg-white rounded-xl shadow-lg text-gray-800">
            <h2 className="text-2xl font-bold mb-6 text-center text-indigo-700 uppercase tracking-tight">
            ...
            {/* TOPIC MANAGER */}
            <div className="p-5 bg-indigo-50 border border-indigo-100 rounded-3xl mb-8">
                <h3 className="font-black text-indigo-800 mb-3 uppercase text-xs tracking-widest">📚 Topic Manager</h3>
                <div className="flex gap-2 mb-4">
                    <input
                        className="flex-grow border rounded-xl px-4 py-2 text-sm outline-none focus:ring-2 focus:ring-indigo-400"
                    ...
                    <button
                        ...
                        className="bg-indigo-600 text-white px-6 py-2 rounded-xl text-sm font-bold hover:bg-indigo-700 disabled:bg-gray-300"
                ...
                <div className="flex flex-wrap gap-2">
                    {topics.map((t, i) => (
                        <span key={i} className="px-3 py-1 bg-white border border-indigo-200 text-indigo-700 rounded-full text-[10px] font-black uppercase">
                            {t}
                        </span>
                    ))}
                </div>
            ...
            <div className="bg-white p-6 border rounded-3xl shadow-sm mb-8">
                ...
                <textarea
                    className="w-full border rounded-2xl p-4 mb-4 text-sm focus:ring-2 focus:ring-green-400 outline-none"
                    rows="3"
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3 mb-4">
                    {["A", "B", "C", "D"].map((o) => (
                        <div key={o} className="flex items-center gap-2">
                            <span className="font-black text-gray-300">{o}</span>
                            <input
                                className="flex-grow border rounded-xl px-3 py-2 text-sm"
                ...
                <div className="flex flex-col sm:flex-row justify-between items-center gap-4">
                    <div className="flex items-center gap-2">
                        <span className="text-xs font-black uppercase text-gray-400">Correct:</span>
                        <select
                            className="border rounded-xl px-4 py-1 text-sm font-bold"
                    ...
                    <div className="flex gap-2 w-full sm:w-auto">
                        ...
                        <button
                            ...
                            className={`flex-grow sm:flex-none px-8 py-2 rounded-xl text-xs font-bold uppercase tracking-widest text-white shadow-md ${
                                editingQuestionIndex !== null ? "bg-orange-500" : "bg-green-600"
                            } disabled:bg-gray-300`}
                        >
                            {submitting ? "Saving..." : editingQuestionIndex !== null ? "Update" : "Submit"}
                        </button>
                    </div>
                </div>
            </div>
                    ...
                    <tbody className="divide-y divide-gray-50">
                        {fetchedQuestions.map((q, i) => (
                            <tr key={i} className="hover:bg-gray-50 transition-colors">
                                ...
                                <td className="p-4 font-bold text-indigo-600 uppercase text-[10px]">{q.topic}</td>
                                <td className="p-4 text-gray-700">{q.question}</td>
                                <td className="p-4 font-black text-green-600">{q.answer}</td>
                                <td className="p-4">
